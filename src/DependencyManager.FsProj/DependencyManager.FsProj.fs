...
module FsProjDependencyManager =
    ...
    let loadProjects (baseDir: DirectoryInfo) (projects: FileInfo list) =
        if List.isEmpty projects then
            {
                FsProjDependencies = []
                Notifications = [| "No projects to load" |]
            }
        else
            ...
            let existingProjects, notFoundProjects =
                projects |> List.partition (fun fi -> fi.Exists)
    ...
    let reAssemblyAttributes =
        Regex @"(\.NETCoreApp|\.NETStandard),Version=v\d+\.\d+\.AssemblyAttributes.fs$"

    let reAssemblyInfo projectName =
        Regex @$"{projectName}.AssemblyInfo.fs$"
    ...
    let collectDependencies (result: FsProjDependenciesResult) =
...

        {
            ...
            References =
                projsDependencies
                |> List.collect (fun p -> filterOutFrameworkReferences p.References)
                |> List.distinct
            Sources =
                projsDependencies
                |> List.collect (fun p -> filterOutGeneratedSourceFiles p.ProjectPath p.Sources)
                |> List.distinct
    ...
    let resolveDependencies (baseDir: DirectoryInfo) (projects: FileInfo list) = loadProjects baseDir projects

    type DummyForGettingAssmbly() =
        do ()

    let getDependencyManagerPath () =
        typeof<DummyForGettingAssmbly>.Assembly.Location
    ...
    let resolveDependenciesOutOfProcess (baseDir: DirectoryInfo) (projects: FileInfo list) =
        ...
        let projectsAsArgs =
            projects |> List.map (fun fi -> fi.FullName) |> String.concat " "
    ...
    member _.ResolveDependencies
        ...
        try
            ...
            let generatedScriptBody =
                FsProjDependencyManager.makeScriptFromReferences references
